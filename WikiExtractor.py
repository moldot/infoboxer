...
def process_data(input, output):
    ...
    for line in input:
        ...
        if tag == 'page':
            counter += 1
            if (counter % 100 == 0): print '>>> At page ', counter
            page = []
            redirect = False
            ...
            if (colon < 0 or title[:colon] in acceptedNamespaces) and \
                ...
                sys.stdout.flush()
                infoboxes = extract_infoboxes(page)
                update_infobox_summary(infobox_summary, infoboxes, id, title)
                WikiDocument(output, id, title, infoboxes, ''.join(page))
            id = None
            page = []
            infoboxes = []
        elif tag == 'base':
    ...
    print 'Number of pages: ' + str(counter)
    print infobox_summary_to_string(infobox_summary)

### INFOBOX HANDLING #####################################################

def extract_infoboxes(page):
    ...
    for line in page:
        ...
        infobox = infobox_parser.get_class(match[0].replace('{{', '')).replace('wikipedia-', '')
